  ...
  pesquisa.p_login(sess.email, sess.senha, function (error, logado) {
    if (logado.length == 0) {
      ...
      pesquisa.p_getAlunos(logado[0].id_professor, function (err, getAlunos) {
        pesquisa.p_getAllSalas(logado[0].id_professor, function (err2, salas) {
          pesquisa.p_getSalasGrupo(logado[0].id_professor, function (err3, salasGrupo) {
            res.render('dashboard/dashboard', {
              validacao: {},
              user: logado,
              othersInfos: getAlunos,
              salas: salas,
              salasEgrupos: salasGrupo
            });
          });
        });
      });
...
// CADASTRAR Salas
module.exports.cadastrar_sala = function (app, req, res) {
  sess = req.session;
  let user = req.body;
  ...
  req.assert('nome_sala', 'Precisamos saber o nome do aluno!').notEmpty();
  ...
  if (erros) {
    ...
    pesquisa.p_login(sess.email, sess.senha, function (error1, idProfessor) {
      pesquisa.p_getAlunos(idProfessor[0].id_professor, function (error2, getAlunos) {
        pesquisa.p_inSalas(user.nome_sala, idProfessor[0].id_professor, function (error3, salaCadastrada){
          pesquisa.p_getAllSalas(idProfessor[0].id_professor, function (err2, salas) {
            pesquisa.p_getSalasGrupo(idProfessor[0].id_professor, function (err3, salasGrupo) {
              if (error1 || error2 || error3) {
                res.render('dashboard/dashboard', {
                  validacao: [{
                    titulo: 'Ops!',
                    msg: 'Não conseguimos cadastrar sua sala',
                    color_background: 'var(--vermelho)'
                  }],
                  user: idProfessor,
                  othersInfos: getAlunos,
                ...
                res.render('dashboard/dashboard', {
                  validacao: [{
                    titulo: 'Sala cadastrada com sucesso!',
                    msg: 'Agora cadastre seus grupos',
              ...
              pesquisa.p_getUltimoGrupo(function (err3, idGrupo) {
                pesquisa.p_getIdSala(user.select_sala, function (err4, idSala) {
                  pesquisa.p_inRegistros(idGrupo[0].id_grupo, p_getAlunos[0].id_aluno, function (err5, registrado) {
                    pesquisa.p_inRegistrosSala(idSala[0].id_sala, idGrupo[0].id_grupo, function (err6, salaRegistrada) {
                      pesquisa.p_getAllSalas(idProfessor[0].id_professor, function (errorGet1, salas) {
                        pesquisa.p_getSalasGrupo(idProfessor[0].id_professor, function (errorGet2, salasEgrupos) {
                          if (error || error2 || err1 || err2 || err3 || err4 || err5 ||err6) {
   ...
   pesquisa.p_login(sess.email, sess.senha, function (error, idProfessor) {
     pesquisa.p_getAlunos(idProfessor[0].id_professor, function(error2, p_getAlunos){
       pesquisa.p_uploadFoto(img, idProfessor[0].id_professor, function (err, updloaded) {
         pesquisa.p_getAllSalas(idProfessor[0].id_professor, function (errGet, salas) {
           pesquisa.p_getSalasGrupo(idProfessor[0].id_professor, function (errGet2, salasEgrupos) {
             if (err) {
               console.log(err);
               res.render('dashboard/dashboard',{
                 validacao: [{
